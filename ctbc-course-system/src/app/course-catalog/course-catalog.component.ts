...
export class CourseCatalogComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // 從 URL Query Params 還原搜尋狀態 (重整不會清)
    this.route.queryParamMap.subscribe(params => {
      ...
      this.selectedCategoryId = cid ? Number(cid) : undefined;
  ...
  goLast(): void {
    ...
    if (lastPage >= 0 && this.page !== lastPage) {
      this.page = lastPage;
      this.syncQueryParams();
    }
  }
  ...
  addFavorite(courseId: number) {
    this.favoriteService.add(courseId).subscribe({
      next: () => alert('已加入收藏'),
      error: err => {
        if (err.status === 403) {
          alert('只有 USER 可以收藏');
